...
//QS：为什么把括号中的传给icp_ptr,传给的icp_ptr是什么
//QS: 为什么NEW之后没有考虑delete
ICPRegistration::ICPRegistration(const YAML::Node& node)
    ...
    SettRegistrationParam(max_dist, trans_eps, eculi_eps, max_iter);
}
...
bool ICPRegistration::SettRegistrationParam(float max_dist, float trans_eps, float eculi_eps, int max_iter) {
